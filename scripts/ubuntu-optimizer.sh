...
install_pkgs() {
    if ! sudo apt -y install "$@"; then
        local pkg
        for pkg in "$@"; do
            sudo apt -y install "$pkg" || red_msg "Package not available, skipped: $pkg"
        done
    fi
}
...
installations() {
    ...
    install_pkgs bc binutils binutils-common binutils-x86-64-linux-gnu ubuntu-keyring haveged jq libsodium-dev libsqlite3-dev libssl-dev packagekit qrencode socat
...
sysctl_optimizations() {
    ...
    ## Add new parameters. Read More: https://github.com/LivingG0D/linux-optimizer/blob/main/files/sysctl.conf
